...
if exist(INFO.file.conv_name{isess});
...
if strcmp(INFO.file.extension_type,'oxy3')==1
    INFO.conv.system='oxy3';
end

% Conversion INFO.files to files usable in this function
if isfield(INFO.conv,'system')==0 || isfield(INFO.conv,'fs')==0 || isfield(INFO.conv,'dist')==0 || isfield(INFO.conv,'wavelength')==0 || isfield(INFO.conv,'DPF')==0 || isfield(INFO.conv,'flag_DPF_corr')==0 || isfield(INFO.conv,'total_ch')==0 || isfield(INFO.conv,'ext_coef')==0 || isfield(INFO.conv,'ch_config')==0;
    error('Not all INFO components necessary to run data conversion are available. Add the missing components to the INFO-file to continue.');
else
    fname_nirs      =INFO.file.Sess(isess).rawdata_dir{INFO.counter.iSubj};
    system          =INFO.conv.system;
    fs              =INFO.conv.fs;
    dist            =INFO.conv.dist;
    wavelength      =INFO.conv.wavelength;
    DPF             =INFO.conv.DPF;
    flag_DPF_correction =INFO.conv.flag_DPF_corr;
    total_ch        =INFO.conv.total_ch;
    ext_coef        =INFO.conv.ext_coef;
    ch_config       =INFO.conv.ch_config;
end

if strcmp(INFO.file.extension_type,'mat')==0 %No data conversion required
    switch system
        case 'oxy3'
            if isempty(which('oxysoft2matlab')); error('oxysoft2matlab file not in path: needed to read in oxy3-files'); end
            nirs_data_temp=oxysoft2matlab(fname_nirs,'oxy/dxy',[],'false');
            nirs_data=oxysoft2matlab(fname_nirs,'nirs-spm',[]);
            nirs_data.ADvalues=nirs_data_temp.ADvalues;
            nirs_data.SampleTime=nirs_data_temp.metaInfo.SampleTime;
            clear nirs_data_temp
            if INFO.conv.fs~=nirs_data.fs
                disp(['The fs given in the INFO file (',num2str(INFO.conv.fs),') did not match the fs in the oxymon-data (',num2str(nirs_data.fs),'). INFO.fs is changed to (',num2str(nirs_data.fs),')']);
                INFO.conv.fs=nirs_data.fs;
            end
            if strcmp(num2str(INFO.conv.wavelength),num2str(nirs_data.wavelength))==0
                disp(['The wavelength given in the INFO file (',num2str(INFO.conv.wavelength),') did not match the wavelength in the oxymon-data (',num2str(nirs_data.wavelength),'). INFO.wavelength is changed to (',num2str(nirs_data.wavelength),')']);
                INFO.conv.wavelength=nirs_data.wavelength;
            end
            INFO.conv.total_ch=nirs_data.nch;
            close all
        case 'oxymon' % OXYMON MK III
            if isempty(fname_nirs)==1 | isempty(system)==1 | isempty(fs)==1 | isempty(dist)==1 | isempty(wavelength)==1 | isempty(DPF)==1 | isempty(flag_DPF_correction)==1;
            ...
            nirs_data = DNB_nir_to_mat(fname_nirs, fs, dist, wavelength, DPF, flag_DPF_correction);
            nirs_data.wavelength = wavelength;
            nirs_data.distance = dist;
            nirs_data.DPF = DPF;
            nirs_data.nch = size(nirs_data.oxyData, 2); INFO.conv.total_ch=nirs_data.nch;
            if flag_DPF_correction == 1
    ...
    INFO.file.conv_name{isess} = INFO.file.Sess(isess).rawdata_dir{INFO.counter.iSubj};
end

if isempty(INFO.conv.total_ch);
    error('Total number of channels should be entered in the INFO file (INFO.conv.total_ch).');
end

INFO.conv.total_ch=nirs_data.nch;

% Save converted file, oxy3 system saves automatically
