function [y, noMARA] = MARA_spm_fnirs(x,fs,Thres,L,alpha)
...
%           deviation (MSD)
% Thres:    Threshold for artifact detection (mean signal multiplier)
...
% (1) Artefact detecion
k = round(L/2);
[A_Idx,s2_1,s2_2] = MADetection(x,k,Thres,fs,alpha);
if isnan(A_Idx)==1 & isnan(s2_1)==1 & isnan(s2_2)==1        % Return without MARA if moving standard deviation signal does not reach threshold 
    y=x; noMARA=1; return;
end
if isempty(A_Idx)==1
        y=x; noMARA=1; return;                                 % Return when no artefacts were found except for the beginning.
end
...
function [A_Idx,s2_1,s2_2] = MADetection(x,L,Thres,fs,alpha);
...
%%%%%%%%%%
indx_n = find(isnan(s2) == 1); s3=s2; s3(indx_n) = [];
means=mean(s3);                 
T=means*Thres;              % Calculate threshold as mean*INFO.MARA.T
%%%%%%%%%%

if max(s2) < T
%     disp(['--->   Please choose a propper T value! T must be < ' num2str(max(s2))])
    A_Idx=NaN; s2_1=NaN; s2_2=NaN; return;
...
% (4) Detection of the beginning indices and end indices of the artefact
for i = 1:length(s2_2);
    if i < length(s2_2);   
...
d = find (q1>0);
if strcmp(p1{d(1)},'stop')==1       %When there is an end indice at the start of the signal without a beginning indice, leave it.
    d(1)=[];
end
A_Idx = q1(d);
