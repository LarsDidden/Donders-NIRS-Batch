function [converted_data] = DNB_nir_to_mat(filen,fs, dist, wavelength, DPF, flag_DPF_correction)
% Convert oxy-data acquired during data acquisition to mat-file.
%
...
alignData = zeros(rawdataLength,INFO.conv.total_ch*lightNum);               % channel number : 24, light source number : 2

for(iterChannel = 1:INFO.conv.total_ch)
...
for(iterCol = 1:INFO.conv.total_ch*lightNum)
...
oxyData = zeros(rawdataLength,INFO.conv.total_ch);
dxyData = zeros(rawdataLength,INFO.conv.total_ch);
for (iterCol = 1:INFO.conv.total_ch)
