...
if nargin == 1 && isstruct(K)
    ...
    %-------------------------------------------------------------------
    if isstruct(K)
         Y2=sparse(size(Y,1),size(Y,2)); % Preallocating Y2
        ...
        for s = 1:length(K)
            ...
            if INFO.counter.iFilt==2
                ...
                varnames={var.name};
                varnamesnew=[];
                keepvar={'Y2','y','tmpsave','rowidx','varnamesnew'};
                cntr=1;
                ...
                keep Y2 y tmpsave rowidx varnamesnew;
                Y2(rowidx,:) = y;
                clear y
                load(tmpsave,varnamesnew{:}); delete tmpsave
                toc
            end
        end
