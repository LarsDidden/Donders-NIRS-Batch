...
if isfield(INFO.stat,'STAT')==0 || isfield(INFO.stat,'spec_hemi')==0 || isfield(INFO.stat,'p_value')==0 || isfield(INFO.stat,'correct_p')==0;
    ...
    filen_SPM       = INFO.file.pp_name;
    STAT            = INFO.stat.STAT;
    spec_hemi       = INFO.stat.spec_hemi;
    p_value         = INFO.stat.p_value;
    correct_p       = INFO.stat.correct_p;
    disp_fig        = INFO.plots;
end
% Don't make new stat files (like interp_matrix/interp_beta/interp_cov) if
% these files already exist for this subject. Mostly important for
% debugging.
file_stat_replace='yes';

for iCon=1:numel(INFO.stat.con)
    for iView = 1:numel(INFO.stat.spec_hemi)

        if isempty(filen_SPM) == 1
            disp('Please specify the file resulting from the estimation process as an input variable.');
            return;
        end
        load(filen_SPM);
        dir_spm = INFO.file.stat_dir;
        ...
        xCon(1,Ic) = struct('name', INFO.stat.con(iCon).name, 'STAT', STAT, 'c', INFO.stat.con(iCon).vec, 'X0', [], 'iX0', [], 'X1o', [], 'eidf', [], 'Vcon', [], 'Vspm', []);
        SPM_nirs.xCon = xCon;
        
        load(fullfile(INFO.file.coor_name)); % load channel information
        
        % specify the view of rendered brain
        for iView = 1:numel(INFO.stat.spec_hemi)
            switch INFO.stat.spec_hemi{iView}
                case ''
            ...
            if isfield(preproc_info, 'ch_set') == 1
                ...
                for kk = 1:total_set
                    if strcmp(INFO.SCI.check,'yes')==1 
                        index = unique([find(rchn(1:size(preproc_info.ch_set(kk).ch,2)) ~= -1) find(cchn(1:size(preproc_info.ch_set(kk).ch,2)) ~= -1)]);
                        chs{kk}  = [1:size(preproc_info.ch_set(kk).ch(index),2)];
                        name{kk} = preproc_info.ch_set(kk).name;
                    else
                        index = unique([find(rchn(1:INFO.conv.total_ch) ~= -1) find(cchn(1:INFO.conv.total_ch ~= -1))]);
                        chs{kk}  = [1:INFO.conv.total_ch];
                        name{kk} = preproc_info.ch_set(kk).name;
                    end
                end
            ...
            clear SPM_nirs.nirs.beta2 SPM_nirs.nirs.ResSS2 SPM_nirs.nirs.res2
            %------------------------------------------------------------------
            ...
            if strcmp(file_stat_replace,'yes')==0               % Don't replace stat_files if no
                filen_interp_matrix = [dir_spm filesep 'interp_matrix_' spec_hemi{iView} '_' SPM_nirs.nirs.Hb '_' INFO.stat.con(iCon).fname  '.mat'];
            ...
            if isempty(B) == 1 || isempty(index_mask_set) == 1
                ...
                save([dir_spm filesep 'interp_matrix_' spec_hemi{iView} '_' SPM_nirs.nirs.Hb '_' INFO.stat.con(iCon).fname '.mat'], 'B', 'index_mask_set');
                % save gradient of interpolating kernels in x and y direction
                save([dir_spm filesep 'interp_matrix_grad_' spec_hemi{iView} '_' SPM_nirs.nirs.Hb '_'  INFO.stat.con(iCon).fname '.mat'], 'Bx', 'By', 'index_mask_set');
            ...
            filename_beta = [dir_spm filesep 'interp_beta_' spec_hemi{iView} '_' SPM_nirs.nirs.Hb '_' INFO.stat.con(iCon).fname '_' INFO.dataselect.subjectnow '.mat'];
            filename_cov = [dir_spm filesep 'interp_cov_' spec_hemi{iView} '_' SPM_nirs.nirs.Hb '_' INFO.stat.con(iCon).fname '_' INFO.dataselect.subjectnow '.mat'];
            ...
            if isempty(interp_beta) == 1 || isempty(interp_var) == 1 || isempty(index_mask) == 1
                ...
                for kk = 1:total_set
                    if isempty(B{kk}) == 0 % interpolating matrix exists
                        ...
                        mtx_var = zeros(nch); %size(SPM_nirs.nirs.beta,2));
                        for aa = 1:nch
                            for bb = 1:nch
            ...
            if strcmp(file_stat_replace,'yes')==0               % Don't replace stat_files if no
                ...
                filename = [dir_spm filesep 'interp_' xCon(Ic).STAT '_' spec_hemi{iView} '_' SPM_nirs.nirs.Hb '_' INFO.stat.con(iCon).fname '.mat'];
                fid = fopen(filename);
                if fid ~= -1 % if exists
                    fclose(fid);
                    try
                ...
                switch xCon(Ic).STAT
                    case 'T' % t-statistic calculation
                        ...
                        if isempty(Bx) == 1 || isempty(By) == 1
                            load([dir_spm filesep 'interp_matrix_grad_' spec_hemi{iView} '_' SPM_nirs.nirs.Hb '_'  INFO.stat.con(iCon).fname '.mat']);
                        end
                        ...
                        filename = [dir_spm filesep 'interp_T_' spec_hemi{iView} '_' SPM_nirs.nirs.Hb '_' INFO.stat.con(iCon).fname '.mat'];
                    case 'F'
                        [stat df] = calc_F_stat(SPM_nirs, B, chs, xCon(Ic).c);
                        % save interpolated F-statistics
                        filename = [dir_spm filesep 'interp_F_' spec_hemi{iView} '_' SPM_nirs.nirs.Hb '_'  INFO.stat.con(iCon).fname '.mat'];
                end
                stat = stat(:);
        ...
        if strcmp(disp_fig,'yes')==1
            ...
            axis off
            axis image
            title([INFO.dataselect.subjectnow,' ',INFO.model.hb,' ' STAT ' map: ',INFO.stat.con(iCon).name],'Fontsize',15);
            hc = colorbar;
        ...
        % Save T/P-map
        tp_map_name         = ['Brain maps'];
        tp_rootdir  = fullfile(INFO.file.dir,INFO.dataselect.subjectnow,INFO.dataselect.taskname,'Combined_Session','Figures',INFO.file.name,tp_map_name);
        if ~exist(tp_rootdir); mkdir(tp_rootdir); end
        tp_path              = fullfile(tp_rootdir,[fnamecon,'_unthresholded_',INFO.stat.spec_hemi{iView}]);
        saveas(gcf,tp_path,INFO.extension);
        close(gcf);
        % update & save the SPM_file
        save_dir = fullfile(INFO.file.stat_dir,fnamecon);
        ...
        switch correct_p
            case 'EC'
                threshold = DNB_calc_EC(SPM_nirs.nirs.LKC{side_hemi}, p_value, SPM_nirs.xCon(Ic).STAT, df);
            case 'tube'
        ...
        if strcmp(disp_fig,'yes')==1
            ...
            axis off
            axis image
            title([INFO.dataselect.subjectnow,' ',INFO.model.hb,' (p<' num2str(p_value) ', ' correct_p ' correction: ',num2str(numel(index_over)),' voxels): ',INFO.stat.con(iCon).name],'FontSize',15)
            try
                hc = colorbar;
                set(hc, 'YLim', [sbar(65) sbar(128)]);
                y_tick = linspace(sbar(65), sbar(128), 5)';
        ...
        % Save activation-map
        activ_path              = fullfile(tp_rootdir,[fnamecon,'_thresholded_',INFO.stat.spec_hemi{iView}]);
...
% Save INFO-variable to file
infoname = [INFO.file.name,'_INFO'];
save_dir2 = fullfile(INFO.file.info_dir,infoname);
save(save_dir2, 'INFO');
